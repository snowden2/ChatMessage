...
class AddFriendsCell: UITableViewCell {
	...
	func loadImage(user: FUser, tableView: UITableView, indexPath: IndexPath) {
		...
		if let path = DownloadManager.pathUser(user.objectId()) {
			...
			downloadImage(user: user, tableView: tableView, indexPath: indexPath)
		}
	}

	//---------------------------------------------------------------------------------------------------------------------------------------------
	func downloadImage(user: FUser, tableView: UITableView, indexPath: IndexPath) {

		DownloadManager.startUser(user.objectId(), pictureAt: user.pictureAt()) { path, error, network in
			...
			if ((indexSelf == nil) || (indexSelf == indexPath)) {
				if (error == nil) {
					...
					DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
						self.downloadImage(user: user, tableView: tableView, indexPath: indexPath)
					}
				}
			}
		}
